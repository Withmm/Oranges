...
PRIVATE void new_dir_entry(struct inode *dir_inode, int inode_nr,
			   char *filename);
...
PUBLIC int do_open()
{
	...
	phys_copy((void *)va2la(TASK_FS, pathname),
		  (void *)va2la(src, fs_msg.PATHNAME), name_len);
	pathname[name_len] = 0;
	...
	if (pin) {
		...
		if (imode == I_CHAR_SPECIAL) {
			...
			send_recv(BOTH, dd_map[MAJOR(dev)].driver_nr,
				  &driver_msg);
...
PRIVATE struct inode *create_file(char *path, int flags)
{
	...
	int free_sect_nr =
		alloc_smap_bit(dir_inode->i_dev, NR_DEFAULT_FILE_SECTS);
	struct inode *newino =
		new_inode(dir_inode->i_dev, inode_nr, free_sect_nr);
	...
	for (i = 0; i < sb->nr_smap_sects; i++) { /* smap_blk0_nr + i :
		...
		for (j = 0; j < SECTOR_SIZE && nr_sects_to_alloc > 0; j++) {
			...
			if (!free_sect_nr) {
				...
				free_sect_nr = (i * SECTOR_SIZE + j) * 8 + k -
					       1 + sb->n_1st_sect;
			}

			for (; k < 8;
			     k++) { /* repeat till enough bits are set */
