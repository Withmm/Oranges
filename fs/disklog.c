...
#define DISKLOG_RD_SECT(dev, sect_nr)                     \
	rw_sector(DEV_READ, dev, (sect_nr) * SECTOR_SIZE, \
		  SECTOR_SIZE, /* read one sector */      \
		  getpid(), logdiskbuf);
#define DISKLOG_WR_SECT(dev, sect_nr)                      \
	rw_sector(DEV_WRITE, dev, (sect_nr) * SECTOR_SIZE, \
...
PUBLIC int disklog(char *logstr)
{
	...
	int nr_log_blk0_nr =
		sb->nr_sects - NR_SECTS_FOR_LOG; /* 0x9D41-0x800=0x9541 */
	...
	if (!pos) { /* first time invoking this routine */
		...
		int sect_nr = smap_blk0_nr +
			      nr_log_blk0_nr / bits_per_sect; /* 3+9=12 */
		...
		for (i = 0; i < sect_cnt; i++) {
			DISKLOG_RD_SECT(device,
					sect_nr +
						i); /* DISKLOG_RD_SECT(?, 12) */

			for (; byte_off < SECTOR_SIZE && bits_left > 0;
			     byte_off++) {
				for (; bit_off < 8;
				     bit_off++) { /* repeat till enough bits are set */
					assert(((logdiskbuf[byte_off] >>
						 bit_off) &
						1) == 0);
		...
		int chunk =
			min(MAX_IO_BYTES, LOGDISKBUF_SIZE >> SECTOR_SIZE_SHIFT);
		assert(chunk == 256);
		int sects_left = NR_SECTS_FOR_LOG;
		for (i = nr_log_blk0_nr; i < nr_log_blk0_nr + NR_SECTS_FOR_LOG;
		     i += chunk) {
			memset(logdiskbuf, 0x20, chunk * SECTOR_SIZE);
			rw_sector(DEV_WRITE, device, i * SECTOR_SIZE,
				  chunk * SECTOR_SIZE, getpid(), logdiskbuf);
			sects_left -= chunk;
		}
		if (sects_left != 0)
			panic("sects_left should be 0, current: %d.",
			      sects_left);
...
PUBLIC void dump_fd_graph(const char *fmt, ...)
{
	...
	logbufpos +=
		sprintf(logbuf + logbufpos, "		rankdir = \"LR\"\n");
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++, p_proc++) {
		...
		logbufpos +=
			sprintf(logbuf + logbufpos,
				"\t\t\"proc%d\" [\n"
				"\t\t\tlabel = \"<f0>%s (%d) "
				"|<f1> p_flags:%d(%s)"
				"|<f2> p_parent:%d%s"
				"|<f3> eip:0x%x",
				i, p_proc->name, i, p_proc->p_flags,
				proc_flags[p_proc->p_flags], p_proc->p_parent,
	...
	logbufpos +=
		sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0>ANY |<f1> ");
	logbufpos += sprintf(logbuf + logbufpos, "\t\"\n");
	...
	logbufpos +=
		sprintf(logbuf + logbufpos, "\t\tlabel = \"filedescs\";\n");
	logbufpos += sprintf(logbuf + logbufpos, "\t}\n");
#endif
	...
	for (i = 0; i < NR_INODE; i++) {
		...
		logbufpos +=
			sprintf(logbuf + logbufpos, "\t\t\"inode%d\" [\n", i);
		logbufpos +=
			sprintf(logbuf + logbufpos,
				"\t\t\tlabel = \"<f0>inode %d"
				"|<f1> i_mode:0x%x"
				...
				inode_table[i].i_nr_sects, inode_table[i].i_dev,
				inode_table[i].i_cnt, inode_table[i].i_num);
	...
	for (i = 0; i < sb->nr_smap_sects;
	     i++) { /* smap_blk0_nr + i : current sect nr. */
	...
	logbufpos += sprintf(logbuf + logbufpos, "\t\t];\n");
	logbufpos +=
		sprintf(logbuf + logbufpos, "\t\tlabel = \"inode map\";\n");
	logbufpos += sprintf(logbuf + logbufpos, "\t}\n");
#endif
	...
	char *p = _buf;
	for (i = 0; i < SECTOR_SIZE / sizeof(struct inode);
	     i++, p += INODE_SIZE) {
		...
		if (pinode->i_mode) {
			if (pinode->i_start_sect < sb->n_1st_sect) {
				panic("should not happen: %x < %x.",
				      pinode->i_start_sect, sb->n_1st_sect);
			}
	...
	logbufpos +=
		sprintf(logbuf + logbufpos, "\t\tlabel = \"inode array\";\n");
	logbufpos += sprintf(logbuf + logbufpos, "\t}\n");
#endif
