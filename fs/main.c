...
PUBLIC void sync_inode(struct inode *p)
{
	...
	pinode = (struct inode *)((u8 *)fsbuf + (((p->i_num - 1) %
						  (SECTOR_SIZE / INODE_SIZE)) *
						 INODE_SIZE));
...
PRIVATE void do_ls()
{
	...
	for (int i = 0; i < nr_dir_blks; i++) {
		...
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++, pde++) {
			int l = strlen(pde->name);
			phys_copy((void *)va2la(src, p_buf),
				  (void *)va2la(TASK_FS, pde->name), l);
			p_buf += l;
			phys_copy((void *)va2la(src, p_buf),
				  (void *)va2la(TASK_FS, " "), 1);
			++p_buf;
