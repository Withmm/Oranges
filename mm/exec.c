...
PUBLIC int do_exec()
{
	...
	phys_copy((void *)va2la(TASK_MM, pathname),
		  (void *)va2la(src, mm_msg.PATHNAME), name_len);
	...
	if (ret != 0) {
		printl("{MM} MM::do_exec()::stat() returns error. %s",
		       pathname);
		return -1;
	}
	...
	for (i = 0; i < elf_hdr->e_phnum; i++) {
		Elf32_Phdr *prog_hdr =
			(Elf32_Phdr *)(mmbuf + elf_hdr->e_phoff +
	...
	phys_copy((void *)va2la(TASK_MM, stackcopy),
		  (void *)va2la(src, mm_msg.BUF), orig_stack_len);
	...
	phys_copy((void *)va2la(src, orig_stack),
		  (void *)va2la(TASK_MM, stackcopy), orig_stack_len);
