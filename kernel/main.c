...
void exe_helper(char **sub_argv, int *ccount, int *sub_argc, int cmd)
{
...
void shabby_shell(const char *tty_name)
{
	...
	while (1) {
		...
		argv[argc] = 0;
		int ccount = 0;
		char *sub_argv[PROC_ORIGIN_STACK] = { 0 };
		int sub_argc = 0;
		for (int i = 0; i < argc; i++) {
			if (memcmp(argv[i], "&", 2) == 0) {
				exe_helper(sub_argv, &ccount, &sub_argc, 1);
			} else {
		...
		if (sub_argc) {
			exe_helper(sub_argv, &ccount, &sub_argc, 0);
		}
